...
function update() {
  ...
  if (player.isDead) {
    ...
    if (player.isBiting) {
      ...
      player.vy *= 0.5;
      player.biteTimer--;
      if (player.biteTimer <= 0) player.isBiting = false;
    } else if (Math.hypot(dx, dy) > fishRange && !player.isDashing) {
...
function drawFish(x, y, angle, username, hp, maxHp, hasGun, isDead) {
  ...
  if (!isDead) {
    ...
    // name plate background for readability
  ...
  ctx.drawImage(fishImage, -player.width / 2, -player.height / 2, player.width, player.height);

  if (hasGun && !isDead) {
    ctx.drawImage(gunImage, 8, -12, 40, 28);
  }
